...
import {
  type AuthResult,
  withAuthQuery,
} from "@/db/queries/utils/with-auth-query";
...
export const POST = async (req: NextRequest) => {
  ...
  } else if (request?.prefix && request?.bucket && request?.type) {
    result = await withAuthQuery(
      async () => {
        const presignedUrl = createPresignedUrlToDownload({
